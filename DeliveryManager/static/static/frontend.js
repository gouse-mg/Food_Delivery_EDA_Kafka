...
async function loadDishes() {
    try {
        const res = await fetch("http://localhost:8000/res/dishes", {
...
async function addDish(e) {
    ...
    try {
        const res = await fetch("http://localhost:8000/res/dishes/create-dish", {
            method: "POST",
            headers: {
...
function connect() {
    ...
    socket = new WebSocket(`ws://localhost:8000/ws?restaurant_id=${restaurant_id}&token=${token}`);
    ...
    socket.onmessage = (event) => {
        ...
        if (data.flag === "Connected") {
            ...
            handleNewOrder(data);
        } else if (data.flag === "Confirm" || data.flag === "Rejected") {
...
function confirmOrder(oid, menu, li) {
    ...
    li.remove();
    updateOrderStatus(oid, "confirmed");
}
...
async function updateOrderStatus(oid, status) {
    await fetch("http://localhost:8000/OrderStatus", {
        method: "PUT",
        headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${token}`
        },
        body: JSON.stringify({ order_id: oid, status })
    });
}
